...
func (sm *SyncManager) GetRacingDashboard(ctx context.Context, instanceID int, options RacingDashboardOptions) (*RacingDashboard, error) {
	...
	racingTorrents := sm.convertToRacingTorrents(allTorrents, options)
	...
	dashboard.TopFastest = sm.getTopFastest(racingTorrents, options.Limit)

	// Get top ratios
	dashboard.TopRatios = sm.getTopRatios(racingTorrents, options.Limit)

	// Get bottom ratios
	dashboard.BottomRatios = sm.getBottomRatios(racingTorrents, options.Limit)

	// Calculate tracker statistics
	dashboard.TrackerStats = sm.calculateTrackerStats(racingTorrents)
...
func (sm *SyncManager) convertToRacingTorrents(torrents []qbt.Torrent, options RacingDashboardOptions) []RacingTorrent {
	var racingTorrents []RacingTorrent

	for _, torrent := range torrents {
		// Apply filters
		if !sm.matchesFilters(torrent, options) {
			continue
		}
		...
		if torrent.Tracker != "" {
			racingTorrent.TrackerDomain = sm.getDomainFromTracker(torrent.Tracker)
		}
...
func (sm *SyncManager) matchesFilters(torrent qbt.Torrent, options RacingDashboardOptions) bool {
	...
	if len(options.TrackerFilter) > 0 {
		trackerDomain := sm.getDomainFromTracker(torrent.Tracker)
		found := false
...
func (sm *SyncManager) getTopFastest(torrents []RacingTorrent, limit int) []RacingTorrent {
...
func (sm *SyncManager) getTopRatios(torrents []RacingTorrent, limit int) []RacingTorrent {
...
func (sm *SyncManager) getBottomRatios(torrents []RacingTorrent, limit int) []RacingTorrent {
...
func (sm *SyncManager) calculateTrackerStats(torrents []RacingTorrent) TrackerStats {
	stats := TrackerStats{
